...
impl MTFramePayload {
    ...
    pub fn try_into(&self, buffer: &mut Vec<u8>) {
        ...
        match self {
            // MAC
            MAC_DataCnf_AREQ(x) => x.try_into(buffer),
            MAC_DataInd_AREQ(x) => x.try_into(buffer),
            MAC_PurgeCnf_AREQ(x) => x.try_into(buffer),
            MAC_WSAsyncInd_AREQ(x) => x.try_into(buffer),
            MAC_SyncLossInd_AREQ(x) => x.try_into(buffer),
            MAC_AssociateInd_AREQ(x) => x.try_into(buffer),
            MAC_AssociateCnf_AREQ(x) => x.try_into(buffer),
            MAC_BeaconNotifyInd_AREQ(x) => x.try_into(buffer),
            MAC_DisassociateInd_AREQ(x) => x.try_into(buffer),
            MAC_DisassociateCnf_AREQ(x) => x.try_into(buffer),
            MAC_OrphanInd_AREQ(x) => x.try_into(buffer),
            MAC_PollCnf_AREQ(x) => x.try_into(buffer),
            MAC_PollInd_AREQ(x) => x.try_into(buffer),
            MAC_ScanCnf_AREQ(x) => x.try_into(buffer),
            MAC_CommStatusInd_AREQ(x) => x.try_into(buffer),
            MAC_StartCnf_AREQ(x) => x.try_into(buffer),
            MAC_WSAsyncCnf_AREQ(x) => x.try_into(buffer),

            MAC_Init_SREQ(x) => x.try_into(buffer),
            MAC_DataReq_SREQ(x) => x.try_into(buffer),
            MAC_PurgeReq_SREQ(x) => x.try_into(buffer),
            MAC_AssociateReq_SREQ(x) => x.try_into(buffer),
            MAC_AssociateRsp_SREQ(x) => x.try_into(buffer),
            MAC_DisassociateReq_SREQ(x) => x.try_into(buffer),
            MAC_GetReq_SREQ(x) => x.try_into(buffer),
            MAC_SetReq_SREQ(x) => x.try_into(buffer),
            MAC_SecurityGetReq_SREQ(x) => x.try_into(buffer),
            MAC_SecuritySetReq_SREQ(x) => x.try_into(buffer),
            MAC_UpdatePANIdReq_SREQ(x) => x.try_into(buffer),
            MAC_AddDeviceReq_SREQ(x) => x.try_into(buffer),
            MAC_DeleteDeviceReq_SREQ(x) => x.try_into(buffer),
            MAC_DeleteAllDevicesReq_SREQ(x) => x.try_into(buffer),
            MAC_DeleteKeyReq_SREQ(x) => x.try_into(buffer),
            MAC_ReadKeyReq_SREQ(x) => x.try_into(buffer),
            MAC_WriteKeyReq_SREQ(x) => x.try_into(buffer),
            MAC_OrphanRsp_SREQ(x) => x.try_into(buffer),
            MAC_PollReq_SREQ(x) => x.try_into(buffer),
            MAC_ResetReq_SREQ(x) => x.try_into(buffer),
            MAC_ScanReq_SREQ(x) => x.try_into(buffer),
            MAC_StartReq_SREQ(x) => x.try_into(buffer),
            MAC_SyncReq_SREQ(x) => x.try_into(buffer),
            MAC_SetRxGainReq_SREQ(x) => x.try_into(buffer),
            MAC_WSAsyncReq_SREQ(x) => x.try_into(buffer),
            MAC_FHEnableReq_SREQ(x) => x.try_into(buffer),
            MAC_FHStartReq_SREQ(x) => x.try_into(buffer),
            MAC_FHGetReq_SREQ(x) => x.try_into(buffer),
            MAC_FHSetReq_SREQ(x) => x.try_into(buffer),

            MAC_Init_SRSP(x) => x.try_into(buffer),
            MAC_DataReq_SRSP(x) => x.try_into(buffer),
            MAC_PurgeReq_SRSP(x) => x.try_into(buffer),
            MAC_AssociateReq_SRSP(x) => x.try_into(buffer),
            MAC_AssociateRsp_SRSP(x) => x.try_into(buffer),
            MAC_DisassociateReq_SRSP(x) => x.try_into(buffer),
            MAC_GetReq_SRSP(x) => x.try_into(buffer),
            MAC_SetReq_SRSP(x) => x.try_into(buffer),
            MAC_SecurityGetReq_SRSP(x) => x.try_into(buffer),
            MAC_SecuritySetReq_SRSP(x) => x.try_into(buffer),
            MAC_UpdatePANIdReq_SRSP(x) => x.try_into(buffer),
            MAC_AddDeviceReq_SRSP(x) => x.try_into(buffer),
            MAC_DeleteDeviceReq_SRSP(x) => x.try_into(buffer),
            MAC_DeleteAllDevicesReq_SRSP(x) => x.try_into(buffer),
            MAC_DeleteKeyReq_SRSP(x) => x.try_into(buffer),
            MAC_ReadKeyReq_SRSP(x) => x.try_into(buffer),
            MAC_WriteKeyReq_SRSP(x) => x.try_into(buffer),
            MAC_OrphanRsp_SRSP(x) => x.try_into(buffer),
            MAC_PollReq_SRSP(x) => x.try_into(buffer),
            MAC_ResetReq_SRSP(x) => x.try_into(buffer),
            MAC_ScanReq_SRSP(x) => x.try_into(buffer),
            MAC_StartReq_SRSP(x) => x.try_into(buffer),
            MAC_SyncReq_SRSP(x) => x.try_into(buffer),
            MAC_SetRxGainReq_SRSP(x) => x.try_into(buffer),
            MAC_WSAsyncReq_SRSP(x) => x.try_into(buffer),
            MAC_FHEnableReq_SRSP(x) => x.try_into(buffer),
            MAC_FHStartReq_SRSP(x) => x.try_into(buffer),
            MAC_FHGetReq_SRSP(x) => x.try_into(buffer),
            MAC_FHSetReq_SRSP(x) => x.try_into(buffer),

            // RPC
            RPC_MTCommandError(x) => x.try_into(buffer),

            // SYS
            SYS_ResetReq_AREQ(x) => x.try_into(buffer),
            SYS_ResetInd_AREQ(x) => x.try_into(buffer),

            SYS_PingReq_SREQ(x) => x.try_into(buffer),
            SYS_VersionReq_SREQ(x) => x.try_into(buffer),
            SYS_NVCreateReq_SREQ(x) => x.try_into(buffer),
            SYS_NVDeleteReq_SREQ(x) => x.try_into(buffer),
            SYS_NVLengthReq_SREQ(x) => x.try_into(buffer),
            SYS_NVReadReq_SREQ(x) => x.try_into(buffer),
            SYS_NVWriteReq_SREQ(x) => x.try_into(buffer),
            SYS_NVUpdateReq_SREQ(x) => x.try_into(buffer),
            SYS_NVCompactReq_SREQ(x) => x.try_into(buffer),

            SYS_PingReq_SRSP(x) => x.try_into(buffer),
            SYS_VersionReq_SRSP(x) => x.try_into(buffer),
            SYS_NVCreateReq_SRSP(x) => x.try_into(buffer),
            SYS_NVDeleteReq_SRSP(x) => x.try_into(buffer),
            SYS_NVLengthReq_SRSP(x) => x.try_into(buffer),
            SYS_NVReadReq_SRSP(x) => x.try_into(buffer),
            SYS_NVWriteReq_SRSP(x) => x.try_into(buffer),
            SYS_NVUpdateReq_SRSP(x) => x.try_into(buffer),
            SYS_NVCompactReq_SRSP(x) => x.try_into(buffer),

            // UTIL
            UTIL_Loopback_AREQ(x) => x.try_into(buffer),

            UTIL_CallbackSubCmd_SREQ(x) => x.try_into(buffer),
            UTIL_GetExtAddr_SREQ(x) => x.try_into(buffer),
            UTIL_Loopback_SREQ(x) => x.try_into(buffer),
            UTIL_Random_SREQ(x) => x.try_into(buffer),

            UTIL_CallbackSubCmd_SRSP(x) => x.try_into(buffer),
            UTIL_GetExtAddr_SRSP(x) => x.try_into(buffer),
            UTIL_Loopback_SRSP(x) => x.try_into(buffer),
            UTIL_Random_SRSP(x) => x.try_into(buffer),
        }
    }
}
